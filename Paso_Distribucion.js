...
function Paso3_DistribuirAHojasTrabajo() {
  ...
  try {
    ...
    if (hojaContactos && hojaContactos.getLastRow() > 1) {
      ...
      Logger.log(`No se pudo encontrar la hoja "BD Contact Limpia" o está vacía. Se continuará sin datos de contacto.`);
    }
    ...
    if (hojaConsolidado.getLastRow() <= 1) {
      Logger.log("No hay datos en el consolidado final para distribuir. Proceso omitido.");
      return;
    }
...
function mapearFilaParaSeguimiento(filaConsolidado, mapaColumnas, mapaContactos) {
  ...
  // LÓGICA PARA DV
  const runCrudo = filaConsolidado[mapaColumnas['RUN']];
  const run = (runCrudo || '').toString().trim(); // Obtenemos el RUN limpio
  let dv = filaConsolidado[mapaColumnas['DV']] || ''; // Obtenemos el DV del consolidado

  // Si el DV viene vacío PERO SÍ tiene un RUN, lo calculamos
  if ((dv === '' || dv === null) && run !== '') {
    dv = calcularDV(run);
    Logger.log(`DV calculado para RUN ${run}: ${dv}`);
  }
  ...
  return [
    ...
    run, // Usamos el RUN limpio
    dv,  // Usamos el DV (ya sea el original o el recién calculado)                
