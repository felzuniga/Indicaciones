...
function asegurarEncabezados(hoja, encabezados) {
  if (
    ...
    Logger.log("¡Encabezados no encontrados o corruptos! Restaurando...");
    hoja.clear();
...
function calcularDV(run) {
  try {
    let runStr = String(run).replace(/[^0-9]/g, ''); // Limpiar RUN (solo números)
    if (runStr === '') return '';
    ...
    Logger.log(`Error al calcular DV para ${run}: ${e.message}`);
